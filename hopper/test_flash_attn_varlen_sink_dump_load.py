...
def _build_case(seed: int = 20260507):
    ...
    _log("Case config: " f"batch={batch_size}, seqlen_q={seqlen_q}, seqlen_k={seqlen_k}, " f"nheads={nheads}, nheads_k={nheads_k}, headdim={headdim}, dtype={dtype}")
...
def _run_case_fa3(case, use_fa4_sink: bool, enable_sink: bool):
    ...
    if use_fa4_sink and not enable_sink:
        ...
        _log(f"Running FA3 case: use_fa4_sink={effective_use_fa4_sink}, " f"enable_sink={enable_sink}")
...
def _run_case_sgl(case, use_fa4_sink: bool, enable_sink: bool):
    _log("Running SGL kernel version of flash_attn_varlen_func...")
    ...
    out, lse, *rest = flash_attn_varlen_func_sgl(
        q,
        k,
        v,
        _to_cuda_tensor(case["cu_seqlens_q"]),
    ...
    _log(f"SGL kernel run done, enable_sink={enable_sink}")
    return result
...
def _run_case(case, use_fa4_sink: bool, backend: str, enable_sink: bool):
    ...
    if backend == "sgl":
        return _run_case_sgl(case, use_fa4_sink=use_fa4_sink, enable_sink=enable_sink)
...
def _assert_close(name: str, a: torch.Tensor, b: torch.Tensor, atol: float, rtol: float):
    ...
    _log(f"[CHECK] close: {name} | max_diff={diff:.6e}, limit={limit:.6e} " f"(atol={atol:.2e}, rtol={rtol:.2e}, scale={scale:.6e})")
    if diff > limit:
...
def load_and_check(
    ...
    _log(f"Load start <- {path} (backend={backend}, enable_sink={enable_sink}, " f"allow_backend_mismatch={allow_backend_mismatch})")
    ...
    if dumped_backend != backend:
        if allow_backend_mismatch:
            _log(f"[CHECK] backend mismatch allowed: dump backend={dumped_backend}, " f"replay backend={backend}")
        else:
            raise AssertionError(f"Backend mismatch: dump was produced with backend={dumped_backend}, " f"but check requested backend={backend}")

    dumped_enable_sink = payload.get("enable_sink", True)
    if dumped_enable_sink != enable_sink:
        raise AssertionError(f"Sink enable mismatch: dump was produced with enable_sink={dumped_enable_sink}, " f"but check requested enable_sink={enable_sink}")
    ...
    base_tag = f"replay_vs_dump.{backend}_vs_{dumped_backend}" if dumped_backend != backend else "replay_vs_dump"
...
def compare_fa3_vs_sgl(enable_sink: bool = True):
    _log(f"FA3 vs sgl compare start (enable_sink={enable_sink})")
    case = _build_case()

    fa3_off = _run_case(case, use_fa4_sink=False, backend="fa3", enable_sink=enable_sink)
    sgl_off = _run_case(case, use_fa4_sink=False, backend="sgl", enable_sink=enable_sink)
    _assert_finite("compare.fa3.fa4_off", fa3_off)
    _assert_finite("compare.sgl.fa4_off", sgl_off)
    _compare_results("fa3_vs_sgl.fa4_off", fa3_off, sgl_off)

    fa3_on = _run_case(case, use_fa4_sink=True, backend="fa3", enable_sink=enable_sink)
    sgl_on = _run_case(case, use_fa4_sink=True, backend="sgl", enable_sink=enable_sink)
    _assert_finite("compare.fa3.fa4_on", fa3_on)
    _assert_finite("compare.sgl.fa4_on", sgl_on)
    _compare_results("fa3_vs_sgl.fa4_on", fa3_on, sgl_on)

    _compare_results("fa3.fa4_on_vs_off", fa3_on, fa3_off)
    _compare_results("sgl.fa4_on_vs_off", sgl_on, sgl_off)
    _log("FA3 vs sgl compare finished")
...
def test_flash_attn_varlen_sink_dump_load_compare_sgl(tmp_path: Path):
    dump_path = tmp_path / "flash_attn_varlen_sink_case_sgl.pt"
    dump_and_check(dump_path, backend="sgl", enable_sink=True)
...
def _parse_args():
    ...
    parser.add_argument(
        "--mode",
        choices=["dump", "check", "dump_and_check", "compare_fa3_sgl"],
        default="dump_and_check",
        help="dump only, check only, dump then check, or compare FA3 vs sgl directly",
    )
    parser.add_argument(
        "--backend",
        choices=["fa3", "sgl"],
...
def main():
    ...
    if args.mode == "dump":
        ...
        compare_fa3_vs_sgl(enable_sink=args.enable_sink)
...
# python test_flash_attn_varlen_sink_dump_load.py --backend fa3 --mode check --enable-sink --path ./with_sink.pt
# python test_flash_attn_varlen_sink_dump_load.py --backend sgl --mode check --no-enable-sink --path ./no_sink.pt
